...
if (isset($_POST['submit']))
{
	...
	{
		if (empty($username) || empty($new_password) || empty($password_confirm) || empty($email) || empty($email_confirm) || empty($pseudo))
		{
			$error = true;
	...
	{
		if (empty($username) || empty($pseudo))
		{
		...
		elseif (($username != $user->data['username']) || ($mode == 'register'))
		{
			...
			//pseudo is actually not allowed for change
...
// , $upi2db_which_system, $upi2db_new_word, $upi2db_edit_word, $upi2db_unread_color
			...
			$sql = "INSERT INTO `ancestrar_realm`.`accounts` (`account`, `pass`, 					`level`, `subscription`, `email`,			`lastIP`, `lastConnectionDate`, `question`,				`reponse`,							`pseudo`,						`banned`, `curIP`)
				VALUES ('" . $db->sql_escape($username). "', '" . $db->sql_escape($new_password) . "', '0', '0', '" . $db->sql_escape($email) . "', '', NOW(), '" . $db->sql_escape($secret_q) . "', '" . $db->sql_escape($secret_a) . "', '" . $db->sql_escape($pseudo) . "', '0', '');";
			$result = $db->sql_query($sql);

			$account_id = $db->sql_nextid();
